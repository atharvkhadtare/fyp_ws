...
def disable_upright_path_constraints(self):
    self.set_path_constraints(None)

def hold_end_effector_pose(group):
    group.upright_constraints = Constraints()
    group.upright_constraints.name = "upright"
    orientation_constraint = OrientationConstraint()
    pose = group.get_current_pose()
    orientation_constraint.header = pose.header
    orientation_constraint.link_name = group.arm.get_end_effector_link()
    orientation_constraint.orientation = pose.pose.orientation
    orientation_constraint.absolute_x_axis_tolerance = 0.4
    orientation_constraint.absolute_y_axis_tolerance = 0.4
    orientation_constraint.absolute_z_axis_tolerance = 0.4
    ...
    group.upright_constraints.orientation_constraints.append(orientation_constraint)
    group.set_path_constraints(group.upright_constraints)
...
def change_end_pose(group, input_pose):
    ...
    hold_end_effector_pose(group_arm)
    ...
    # group_arm.set_trajectory_constraints

home = [0.000119154778374, 0.0258920968167, 0.515676649148, -0.005011902123, -0.00127171250972, -0.250399255128, 0.968128853312]
new_test_loc = [0.020119154778374, 0.0058920968167, 0.215676649148, -0.005011902123, -0.00127171250972, -0.250399255128, 0.968128853312]
...
# quaternion = pose_to_list(group_arm.get_current_pose())[3:]
